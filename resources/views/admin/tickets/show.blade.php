            ...
            <div class="widget p-4 border rounded">
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-3">
                    ...
                    <div class="text-end">
                        ...
                        <small class="text-muted d-block">Assigned To: {{ $ticket->assignedTo->name ?? '—' }}</small>
                            ...
                            <div class="mt-2">
                                ...
                                <ul class="list-unstyled">
                                    @foreach($reply->attachments as $file)
                                        <li>
                                            <a href="{{ Storage::url($file->filepath) }}" target="_blank">📎 {{ $file->filename }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                ...
                @if($ticket->status !== 'closed')
                    <form method="POST" action="{{ route('admin.tickets.reply', $ticket->id) }}" enctype="multipart/form-data" onsubmit="return syncQuillReply()">
                        @csrf
                        <div class="mb-3">
